...
router.post("/:version/is-there-a-next-of-kin", function (req, res) {
  ...
  const isSpouse = ["Husband", "Wife", "Spouse", "Civil Partner"].includes(
    relationship,
  );
...
router.post("/:version/about-the-spouse-not-spouse", function (req, res) {
  ...
  const isMarried = ["Married", "Civil Partnership"].includes(maritalStatus);
...
router.post("/capture/check-your-answers", function (req, res) {
  const dateOfBirth = normaliseDate(
    req.session.data["deceased-date-of-birth-day"],
    req.session.data["deceased-date-of-birth-month"],
    req.session.data["deceased-date-of-birth-year"],
  );
  const dateOfDeath = normaliseDate(
    req.session.data["date-of-death-day"],
    req.session.data["date-of-death-month"],
    req.session.data["date-of-death-year"],
  );
  const registrationDate = normaliseDate(
    req.session.data["registration-date-day"],
    req.session.data["registration-date-month"],
    req.session.data["registration-date-year"],
  );
...
router.post(
  "/capture/select-who-will-complete-tell-us-once",
  function (req, res) {
...
router.post(
  "/capture/does-the-person-reporting-the-death-want-an-email-confirming-their-tell-us-once-reference-number",
  function (req, res) {
...
router.post("/v1/informer-details/informer-relationship", function (req, res) {
  ...
  if (!relationship || relationship.trim() === "") {
    return res.render("v1/informer-details/informer-relationship", {
      error:
        "Select your relationship to {{ data['deceased-forename']}} {{ data['deceased-surname']}}",
    });
  }
  if (req.session.data.returnTo) {
    const next = req.session.data.returnTo;
...
router.get(
  "/v1/informer-details/informer-same-address-as-deceased",
  function (req, res) {
    req.session.data.returnTo = req.query.returnTo;
...
router.post(
  "/v1/informer-details/select-informer-address",
  function (req, res) {
...
router.post(
  "/v1/informer-details/confirm-informer-address",
  function (req, res) {
...
router.get(
  "/v1/deceased-details/spouse-or-partner/confirm-spouse",
  function (req, res) {
    const data = req.session.data;
    const spouseTerms = ["husband", "wife", "spouse", "civil partner"];

    const informerIsSpouse = spouseTerms.includes(
      (data["informer-relationship"] || "").toLowerCase(),
    );

    const nokIsSpouse = spouseTerms.includes(
      (data["next-of-kin-relationship"] || "").toLowerCase(),
    );
    ...
    if (informerIsSpouse) {
      ...
      res.redirect(
        "/v1/deceased-details/spouse-or-partner/confirm-next-of-kin",
      );
    } else {
